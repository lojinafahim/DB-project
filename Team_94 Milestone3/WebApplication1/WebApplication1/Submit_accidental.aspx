<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="Submit_accidental.aspx.cs" Inherits="WebApplication1.Submit_accidental" %>
    ...
    <form id="form1" runat="server">
        <div class="main-container">
            ...
            <div class="header">
                <br />
                <br />
                <h1>Accidental Leave Request</h1>
                <p>Submit your accidental leave request. All fields marked with * are required. Please provide accurate dates for your leave period.</p>
            </div>
            
            <!-- Note box -->
            <div class="note-box">
                <strong>Important Note:</strong> Accidental leave is for emergencies and unexpected incidents. Please ensure you provide accurate dates and contact HR immediately for urgent cases requiring immediate attention.
            </div>
            ...
            <div class="form-card">
                ...
                <div class="form-row">
                    <div class="form-group">
                        <label for="<%= txtStart.ClientID %>" id="lblStart">Start Date *</label>
                        <asp:TextBox ID="txtStart" runat="server" TextMode="Date" CssClass="form-control"></asp:TextBox>
                        <div class="error-hint" id="errorStart">Please enter a valid start date</div>
                    </div>
                    
                    <div class="form-group">
                        <label for="<%= txtEnd.ClientID %>" id="lblEnd">End Date *</label>
                        <asp:TextBox ID="txtEnd" runat="server" TextMode="Date" CssClass="form-control"></asp:TextBox>
                        <div class="error-hint" id="errorEnd">Please enter a valid end date</div>
                    </div>
                </div>
                
                <!-- Submit Button -->
                <asp:Button ID="btnSubmit" runat="server" CssClass="submit-btn" 
                    Text="Submit Accidental Leave Request" OnClick="btnSubmit_Click" 
                    OnClientClick="return validateForm();" />
                
                <!-- Message display -->
                <div class="message-container">
                    <asp:Label ID="lblMsg" runat="server" CssClass="message-box"></asp:Label>
                </div>
            </div>
        </div>
        ...
        <script>
            ...
            document.addEventListener('DOMContentLoaded', function () {
                ...
                formControls.forEach(function (control) {
                    control.addEventListener('focus', function () {
                        ...
                        var errorId = 'error' + this.id.replace('<%= txtStart.ClientID %>'.replace('txtStart', ''), '').replace('<%= txtEnd.ClientID %>'.replace('txtEnd', ''), '');
                        clearError(errorId);
                    });
                ...
                document.getElementById('<%= txtStart.ClientID %>').addEventListener('change', function () {
                    clearError('errorStart');
                });
                document.getElementById('<%= txtEnd.ClientID %>').addEventListener('change', function () {
                    clearError('errorEnd');
                });
            ...
            function setDefaultDates() {
                var startDate = document.getElementById('<%= txtStart.ClientID %>');
                var endDate = document.getElementById('<%= txtEnd.ClientID %>');
            ...
            function validateForm() {
                ...
                // Validate Start Date
                var startDate = document.getElementById('<%= txtStart.ClientID %>').value;
                if (!startDate) {
                    showError('txtStart', 'errorStart', 'Please enter a start date');
                    isValid = false;
                }
                
                // Validate End Date
                var endDate = document.getElementById('<%= txtEnd.ClientID %>').value;
                if (!endDate) {
                    showError('txtEnd', 'errorEnd', 'Please enter an end date');
                    isValid = false;
                }
                ...
                if (startDate && endDate) {
                    ...
                    if (end < start) {
                        showError('txtEnd', 'errorEnd', 'End date must be after start date');
                        isValid = false;
                    }
                }
